...
class WhoVisitedDataSourceImpl implements WhoVisitedDataSource {
    ...
    try {
      await _firebaseFirestore.runTransaction((transaction) async {
        ...
        if (!visitedUserDoc.exists) {
          throw Exception('Visited user does not exist');
        }

        // Update the visit count in the visited user's document
        transaction
            .update(visitedUserDocRef, {'visitCount': FieldValue.increment(1)});
  ...
  @override
Future<List<UserVisit>> getProfileVisitedProfiles({required String myId}) async {
  try {
    ...
    final visitedUsersSnapshot = await whoVisitedMeRef
        .orderBy('timestamp', descending: true) // Adjust sorting as needed
        .get();
    ...
    final visitedUserIdsWithTimestamps = visitedUsersSnapshot.docs
        .map((doc) {
          final data = doc.data();
          final userId = doc.id;
    ...
    // Extract user IDs
    final visitedUserIds = visitedUserIdsWithTimestamps.map((entry) => entry['userId'] as String).toList();

    // Reference to the 'users' collection
    final usersRef = _firebaseFirestore.collection(FirebaseCollectionConst.users);
    ...
    final visitedUsers = visitedUserIdsWithTimestamps.map((entry) {
      ...
      if (user != null) {
        ...
        return null; // Handle missing user data if necessary
      }
    }).whereType<UserVisit>().toList();
